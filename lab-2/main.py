...
def get_characteristics(generator, sample_size: tuple):
    ...
    for num in sample_size:
        ...
        characteristics[f'{num}']['mean+'] = "[" + str(
            round(mean - np.sqrt(d_mean), 4)) + "; " + str(
            round(mean + np.sqrt(d_mean), 4)) + "]"
        characteristics[f'{num}']['median+'] = "[" + str(
            round(median - np.sqrt(d_median), 4)) + "; " + str(
            round(median + np.sqrt(d_median), 4)) + "]"
        characteristics[f'{num}']['z_R+'] = "[" + str(
            round(z_R - np.sqrt(d_z_R), 4)) + "; " + str(
            round(z_R + np.sqrt(d_z_R), 4)) + "]"
        characteristics[f'{num}']['z_Q+'] = "[" + str(
            round(z_Q - np.sqrt(d_z_Q), 4)) + "; " + str(
            round(z_Q + np.sqrt(d_z_Q), 4)) + "]"
        characteristics[f'{num}']['z_tr+'] = "[" + str(
            round(z_tr - np.sqrt(d_z_tr), 4)) + "; " + str(
            round(z_tr + np.sqrt(d_z_tr), 4)) + "]"
...
def lab2() -> dict[dict]:
    ...
    chars['normal'] = get_characteristics(np.random.standard_normal,
                                          sample_size)
    print("[+] Characteristics recieved.")
    print("[ ] Getting characteristics of Cauchy distribution...")
    chars['cauchy'] = get_characteristics(np.random.standard_cauchy,
                                          sample_size)
    print("[+] Characteristics recieved.")
    print("[ ] Getting characteristics of Student's t distribution...")
    chars['student_t'] = get_characteristics(
        lambda n: np.random.standard_t(df=3, size=n), sample_size)
    print("[+] Characteristics received.")
    ...
    chars['uniform'] = get_characteristics(
        lambda n: np.random.uniform(low=-np.sqrt(3.0), high=np.sqrt(3.0),
                                    size=n), sample_size)
    print("[+] Characteristics recieved.")
    return chars
